...
const workoutTypes = ref(["Push", "Pull", "Legs"]);
</script>

<template>
    <!-- grid -->
    <section class="grid grid-cols-3 gap-4 sm:gridgrid-cols-4">
        <button
            disabled="true"
            @click="handleSelectedWorkout(workoutIdx)"
            :key="workoutIdx"
            v-for="(workout, workoutIdx) in Object.keys(workoutProgram)"
            class="card-button plan-card w-full flex flex-col disabled:shadow-none"
        >
